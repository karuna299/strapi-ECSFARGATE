...
resource "aws_codedeploy_app" "karuna_ecs_app" {
  name             = "karuna-ecs-app"
...
resource "aws_codedeploy_deployment_group" "karuna_ecs_dg" {
  ...
  load_balancer_info {
    target_group_pair_info {
      target_group {
        name = aws_lb_target_group.karuna_tg_blue.name
      }
